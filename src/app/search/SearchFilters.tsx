...
import { MapPin, Calendar as CalendarIcon, Users, Compass, Tent, Star, Search, ArrowRight } from 'lucide-react';
...
export function SearchFilters() {
    ...
    return (
        <div className="space-y-6 max-w-[1200px] mx-auto w-full px-4 md:px-0">
            <form onSubmit={handleFormSubmit} className="flex flex-col lg:flex-row items-stretch gap-2 bg-white p-2 rounded-[2rem] border border-border/10 shadow-xl relative z-20 transition-all">
                ...
                <div className="flex-[1.2] flex items-center gap-4 bg-muted/40 hover:bg-muted/60 px-6 py-3 rounded-[1.5rem] transition-all cursor-pointer group">
                    <div className="h-10 w-10 rounded-xl bg-white flex items-center justify-center shadow-sm transition-transform ring-1 ring-black/5">
                    ...
                    <div className="flex flex-col items-start flex-1 text-left">
                        ...
                        <Select value={city} onValueChange={setCity}>
                            <SelectTrigger className="border-0 bg-transparent p-0 h-auto focus:ring-0 shadow-none font-bold text-primary text-sm w-full tracking-tight">
                                <SelectValue placeholder="All Devbhoomi" />
                            </SelectTrigger>
                            <SelectContent className="bg-white border-black/5 rounded-xl shadow-2xl p-2">
                                <SelectItem value="All" className="font-bold py-3 text-primary text-xs uppercase">All Uttarakhand</SelectItem>
                                {cities?.map((c) => (
                                    <SelectItem key={c} value={c} className="font-bold py-3 text-primary text-xs uppercase">{c}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <div className="flex flex-col items-start flex-1 text-left overflow-hidden">
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                <button type="button" className="text-sm font-bold text-primary truncate hover:text-accent transition-colors text-left w-full tracking-tight uppercase">
                                    {dates?.from ? (
                            ...
                            <PopoverContent 
                                className="w-auto p-0 border-0 bg-white rounded-2xl overflow-hidden shadow-2xl mt-4" 
                                align="center"
                            >
                                <CalendarComponent
                                    mode="range"
                    ...
                    <div className="flex flex-col items-start flex-1 text-left">
                        ...
                        <div className="flex items-center gap-2">
                            <Input 
                                ...
                                className="border-0 bg-transparent p-0 h-auto focus-visible:ring-0 shadow-none font-bold w-8 text-base text-primary tracking-tight"
                            />
                ...
                {/* Execute Button */}
                <Button type="submit" size="lg" className="h-auto bg-accent hover:bg-primary text-accent-foreground hover:text-white rounded-[1.5rem] px-10 text-xs font-black shadow-lg transition-all active:scale-95 py-5 lg:py-0 group uppercase tracking-[0.2em]">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
                {[
                    ...
                    <Button 
                        ...
                        className={cn(
                            "h-10 rounded-full text-[10px] font-black uppercase tracking-[0.2em] px-8 border transition-all flex items-center gap-3",
                            mode === p.id 
