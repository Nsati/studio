...
import { Hotel as HotelIcon, Home, Loader2, Download } from 'lucide-react';
...
function BookingItem({ booking }: { booking: WithId<Booking> }) {
  ...
  const checkInDate = (booking.checkIn as any).toDate();
  const checkOutDate = (booking.checkOut as any).toDate();
        ...
        <div className="flex flex-col flex-grow justify-between p-4 sm:p-6">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm text-muted-foreground">
                ...
                <p><span className="font-semibold text-foreground">Amount Paid:</span> <span className="font-bold text-foreground">{booking.totalPrice.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</span></p>
            </div>
          </div>
